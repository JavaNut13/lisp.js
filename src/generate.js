...
function getUndefined(gener, obj, add) {
  if (obj.type === 'list') {
    var i = 0;
    if (obj.cont[0].cont === 'def') { // Skip the variable we're defining
      i = 2;
    }
    for (; i < obj.cont.length; i++) {
      if (!gener.infix_operators[obj.cont[i].cont]) {
        getUndefined(gener, obj.cont[i], add);
      }
    }
  } else if (obj.type === 'iden') {
    add(obj.cont);
  } else if (obj.type === 'quote') {
    getUndefined(gener, obj.cont, add);
  }
}
